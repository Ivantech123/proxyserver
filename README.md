...
[![Alpine](https://img.shields.io/badge/Alpine-Optimized-brightgreen.svg)]()

Оптимизированный прокси-сервер на базе Alpine Linux с минимальным потреблением ресурсов, поддержкой HTTP/2 и автоматическим управлением.

[Возможности](#-возможности) • [Установка](#-установка) • [Использование](#-использование) • [Оптимизации](#-оптимизации) • [Обновления](#-история-обновлений)
...
<table>
  ...
  <tr>
    ...
    <td>
      • Минимальное потребление RAM<br/>
      • Низкая нагрузка на CPU<br/>
      • Alpine-оптимизация
    </td>
  </tr>
</table>
...
- ✅ Оптимизированный код с минимальным потреблением ресурсов
- ✅ Интеграция с Telegram Stars для монетизации
- ✅ Поддержка Windows и Linux
- ✅ Многоэтапная сборка Docker для минимального размера образов
- ✅ Веб-интерфейс с оптимизированным потреблением ресурсов
...
| Сеть      | 50 Мбит/с   | 100+ Мбит/с   |
| ОС        | Windows 10+ / Ubuntu 20.04+ / Debian 11+ / Alpine 3.16+ | Windows 11 / Ubuntu 22.04 |
...
docker exec proxy-server cat /var/log/haproxy/monitor.log
```
...
- 🔄 Переход на Alpine Linux вместо Debian для уменьшения размера образа на ~70%
- 🔄 Многоэтапная сборка Docker для минимизации конечного образа
- 🔄 Оптимизация настроек HAProxy для снижения потребления памяти
- 🔄 Уменьшение количества процессов и потоков для экономии ресурсов
- 🔄 Эффективное кэширование и сжатие данных

### Оптимизация производительности
- 🔄 Тонкая настройка таймаутов и соединений
- 🔄 Оптимизация сетевого стека для быстрой обработки запросов
- 🔄 Улучшенная система мониторинга с низким потреблением ресурсов
- 🔄 Интеллектуальное управление логами с ротацией

## 📋 История обновлений

### Версия 2.1 (Апрель 2025)
- ✨ Полная оптимизация для снижения потребления ресурсов
- ✨ Переход на Alpine Linux для минимального размера образов
- ✨ Многоэтапная сборка Docker для оптимизации
- ✨ Улучшенный веб-интерфейс с оптимизированным потреблением ресурсов
- ✨ Расширенная система мониторинга и автоматического восстановления
- 🐛 Исправлены проблемы с Unicode в веб-интерфейсе
- 🐛 Улучшена стабильность при высоких нагрузках
